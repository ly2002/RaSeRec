...
class NGCF(GeneralRecommender):
    ...
    def get_norm_adj_mat(self):
        ...
        data = np.ones(inter_M.nnz + inter_M_t.nnz, dtype=np.float32)
        A = sp.coo_matrix((data, (rows, cols)), shape=(self.n_users + self.n_items, self.n_users + self.n_items), dtype=np.float32)
        # norm adj matrix
